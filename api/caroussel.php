<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.next').click(function () { // image suivante
			 ...
			 $img.eq(i).css('display', 'inline'); //on affiche l'image courante
			 $img.eq(i+1).css('margin-left','0px').css('display', 'inline');
			 $img.eq(i+2).css('margin-left','0px').css('display', 'inline');
			 $img.eq(i+3).css('margin-left','0px').css('display', 'inline'); } else {
			 i = 0;
			 }
		 });
		//si on clique sur « prev » ou « < »
		 $('.prev').click(function () { // groupe des images précédentes
			 ...
			 $img.eq(i+1).css('margin-left','0px').css('display', 'inline');
			 $img.eq(i+2).css('margin-left','0px').css('display', 'inline');
			 $img.eq(i+3).css('margin-left','0px').css('display', 'inline');
			 } else {
			 i = 0;
			 }
		 });
